...
def get_exclusions(node):
    ...
    return {"groups":excluded_groups,"classes":excluded_classes,"params":excluded_params}
...
def walk_tree(obj,**kwargs):
    ...
    for group in obj.groups.exclude(name__in=exclusions['groups']+groups_done):
        walk_tree(group,exclusions=exclusions,classes=classes,params=params,groups_done=groups_done)
...
def add_node(**kwargs):
    ...
    node.save()
    return node.id
    #for item in (hostname,description,classes,params,exclusions):
        #if item is not None:
              #node.
...
def optimized_work_tree(obj,**kwargs): 
    ...
    index_pop = to_index.pop; index_extend = to_index.extend;
    while to_index:
        (obj, depth) = index_pop(); objname = obj.name
        if objname in groups_done and groups_done[objname] <= depth:
            continue
...
def optimized_update_values(objs,keyname,valuename,**kwargs):
    ...
    if not depth:
        ...
        results=kwargs.get("results",{"depths":{},"content":{}})
    depths = results["depths"]; content = results["content"]
    for obj in objs:
        if hasattr(obj, keyname):
            keyinstance = getattr(obj, keyname)
            if not depth:
